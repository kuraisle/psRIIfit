...
#' @param belowZero An optional logical value. Defaults to false, meaning intensities bottom out at zero.
#' @return A matrix of intensities for each peak in fixed.
#' @export

fitDemPeaks <- function(fixed, guesses, spectra, wavelengths, belowZero = FALSE){
  ...
  if(belowZero == TRUE){
    ...
    goBelow <- numeric(ncol(fixed))
  }
  myModel <- function(specTimePoint){
    theModel <- mod(modelFormula = myModelFormula, timepoint = specTimePoint, startPars = guesses, bZ = goBelow)
    coef(theModel)
  }
